...
const loadStockInfo1 = async () => {
  const data = await getMisOhlcCG100();
  if (data.rtcode == '0000') {
    let result;
...
const loadOhcl = (cPrice, upper, lower, maxValume, yesPrice, valumeArr) => {
  ...
  if (chart && inpputMaxValue > chart.yAxis[0].max) {
    chart.yAxis[0].max = inpputMaxValue;
    chart = null;
  }
        ...
        <div class="position-relative">
          ...
          >
            <div class="d-flex justify-content-center gap-3">
              <p class="mb-0 fs-12 fw-medium">{{ time }}</p>
              <p class="mb-0 fs-11 fw-medium">{{ TWSE_TXT1 }} {{ i }}</p>
              <p
                class="mb-0 fs-11 fw-medium"
                :style="{ color: dColor }"
              >
                {{ d }}
              </p>
