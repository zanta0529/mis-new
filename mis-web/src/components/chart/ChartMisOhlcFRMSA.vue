...
const handleWheel = (event) => {
  ...
  const chartContainer = document.getElementById('chart-frmsa');
...
onMounted(async () => {
  ...
  await procIndexPageFRMSA();
  emit('stockInfoData', 'frmsa', getStockFRMSAData());
  // ----------------------------目前進度------------------------------
...
const procIndexPageFRMSA = async () => {
  const data = await getMisOhlcFRMSA();
    ...
    if (1 == 1) {
      ...
      chartOptionsTSE.value.yAxis[1] = {
        ...
        ...{
          ...
          tickInterval: Math.ceil(maxValue / 10),
          offset: 0,
          minPadding: 0,
          startOnTick: false,
          endOnTick: false,
        ...
        ...{
          formatter() {
            ...
            if (
              ...
              if (tseLastTimeStamp - 43200000 === this.x && tseLastTime !== '133000') {
...
function updateSeriesName() {
  chartOptionsTSE.value.series[0].name = t('INDEX_FRMSA');
...
<template>
  ...
  >
    <div class="d-flex justify-content-between fw-bold">
      <p class="mx-2">{{ t('INDEX_FRMSA_CHART') }}</p>
