...
const handleWheel = (event) => {
  ...
  const chartContainer = document.getElementById('chart-futures');
...
onMounted(async () => {
  ...
  await initFuturesData();
  emit('stockInfoData', 'futures', getStockFuturesData());
...
const initFuturesData = async () => {
  const data = await getFuturesChart();
  ...
  if (data.rtcode == '0000') {
    if (typeof data.infoArray !== 'undefined' && data.infoArray.length > 0) {
      ...
      const dtA = new Date(exeYear, exeMonth, exeDay, 13, 44, 0);
...
const procFUTURESIndexPage = (data) => {
  ...
  loadChartFutures(channel, d0, upper * 1, lower * 1, prevClosePrice * 1, data);
};
// 時間----------------------------------------------------------
let chart;
let chartChannel;
...
const loadChartFutures = (channel, d0, upper, lower, prevClosePrice, data) => {
  ...
  for (let i = 0; i < dataLength; i++) {
    ...
    if (dataLength - 1 === i) {
      ...
      tseLastTimeStamp = parseInt(data.ohlcArray[i].t);
    }
  ...
  loadOhclFutures(channel, cPrice, volume, upper, lower, maxValue, prevClosePrice);
};
//----------------------------------------------------------
const sPoint = 200;
const loadOhclFutures = (channel, cPrice, volume, upper, lower, maxValue, prevClosePrice) => {
  ...
  if (1 == 1) {
    ...
    chartOptionsTSE.value.yAxis[1] = {
      ...
      ...{
        ...
        tickInterval: Math.ceil(maxValue / 1000) * 100,
        offset: 0,
        minPadding: 0,
        startOnTick: false,
        endOnTick: false,
          ...
          this.points.forEach((point, i) => {
            if (i === 1) {
              if (locale.value === 'zhHant') {
                s += `<br/><span style="color:#0066a0">${point.series.name}: ${point.y} 口</span>`;
              } else {
                s += `<br/><span style="color:#0066a0">${point.series.name}: ${point.y} Vol</span>`;
              }
            } else if (i === 0) {
...
function updateSeriesName() {
  chartOptionsTSE.value.series[0].name = t('INDEX_FUTURES');
  chartOptionsTSE.value.series[1].name = t('INDEX_VOLUME');
}
...
<template>
  ...
  >
    <div class="d-flex justify-content-between fw-bold">
      <p class="mx-2">{{ t('INDEX_FUTURES') }} {{ t('MENU_CHART') }}</p>
