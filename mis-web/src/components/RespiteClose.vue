...
const fetchData = async (currentPage) => {
  try {
    ...
    const tseData = await getStockDelay('tse', '4');
    const otcData = await getStockDelay('otc', '4');
...
const fetchStockInfoData = async (data) => {
  ...
  // 計算漲跌百分比
  const { msgArray } = await useCalculateValue(stockInfoData.msgArray, stockInfoData, { ip: '4' });
  ...
  <div>
    ...
    <div class="container-fluid">
      <div class="row justify-content-between">
        <p class="fs-2 text-black fw-medium p-0 mb-0 col-4">{{ t('POSTPONED_STOCK') }}</p>
    ...
    >
      <template #cell="{ row, thead }">
        ...
        <span v-if="thead.name === t('COMMENT')">{{ t('POSTPONED_STOCK_STRING') }}</span>
      </template>
    </generic-table>
