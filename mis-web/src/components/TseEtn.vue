...
const detailData = ref({
  titleType: 'MENU_CATEGORIES_ETN_TWSE',
...
const theadData = computed(() => {
  return [
    ...
    { name: t('GROUP_ETN_ESTIMATED_N'), key: 'nu', width: '10%', theadAlignment: 'end' },
  ];
});
...
const fetchData = async (type, code, currentPage) => {
  try {
    ...
    let newCode = code;
    if (type === undefined) {
      newType = sessionStorage.getItem('currentType');
    }
    if (code === undefined) {
      newCode = sessionStorage.getItem('currentCode');
    }

    // 更新 Session Storage 中的值
    sessionStorage.setItem('currentType', newType);
    sessionStorage.setItem('currentCode', newCode);

    const start = (currentPage - 1) * count.value;
    const end = start + count.value;

    // 取得現貨類股詳細資料
    const data = await getCategory(newType, newCode);
...
const fetchStockInfoData = async (data) => {
  ...
  const categoryList = data.reduce((accumulator, current) => {
...
const updateDataPeriodically = () => {
  nIntervId = setInterval(() => {
    fetchData('tse', 'B1', currentPage.value);
  }, 5000);
};
onMounted(() => {
  ...
  <div>
    ...
    <div class="container-fluid">
      <div class="row justify-content-between">
        <p class="fs-2 text-black fw-medium p-0 mb-0 col-3">{{ t('MENU_CATEGORIES_ETN_TWSE') }}</p>
        <p class="text-end text-secondary mt-3 mb-0 lh-lg p-0 col-9">
          {{ t('CNT_TRADE_NOTE') }}
        </p>
      </div>
    </div>
    ...
    >
      <template #cell="{ row, thead }">
        ...
        <span v-if="thead.name === t('GROUP_ETN_ESTIMATED_N')">
          <a
            :href="row.nu"
            target="_blank"
            class="link-primary"
    ...
    <manual-table>
      <manual-table-item>{{ t('GROUP_NOTE_ETN') }}</manual-table-item>
      <manual-table-item>{{ t('INDEX_NOTE') }}</manual-table-item>
      <manual-table-item>
        {{ t('FIBEST_NOTE') }}
      </manual-table-item>
