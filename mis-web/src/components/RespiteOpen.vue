...
const theadData = computed(() => {
  return [
    ...
    { name: t('TRADE_VOLUME'), key: 'tv', width: '6%', theadAlignment: 'end' },
    ...
    { name: t('COMMENT'), key: 'ip', width: '7%', theadAlignment: 'end' },
  ];
});
...
const fetchData = async (currentPage) => {
  try {
    ...
    const tseData = await getStockDelay('tse', '5');
    const otcData = await getStockDelay('otc', '5');
    const keysArray = [...Object.keys(tseData.result), ...Object.keys(otcData.result)];

    const data = keysArray.map((key) => {
      return key.replace(/_[^_]+$/, ''); // 使用正則表達式替換底線及其後的值
    });

    totalCount.value = data.length;
    // start: 索引位置 end: 資料取得筆數
    const slicedData = data.slice(start * 1, end * 1);
...
const fetchStockInfoData = async (data) => {
  ...
  // 計算漲跌百分比
  const { msgArray } = await useCalculateValue(stockInfoData.msgArray, stockInfoData, { ip: 5 });
  ...
  <div>
    ...
    <div class="container-fluid">
      <div class="row justify-content-between">
        <p class="fs-2 text-black fw-medium p-0 mb-0 col-4">{{ t('POSTPONED_OPENING') }}</p>
    ...
    <generic-table
      ...
      :title="t('POSTPONED_OPENING')"
      :subtitle="t('UNIT_STR')"
      back-ground-color="background-color: rgba(58, 110, 165, 0.1)"
    >
      <template #cell="{ row, thead }">
        ...
        >
          ...
          >
            {{ row.ex == 'tse' ? t('FIBEST_TSE') : t('FIBEST_OTC') }} {{ row.c }} {{ row.n }}
          </router-link>
        </span>
        <!-- 成交價 -->
        <span
        ...
        <span v-if="thead.name === t('COMMENT')">{{ t('POSTPONED_OPENING_STRING') }}</span>
      </template>
    </generic-table>
