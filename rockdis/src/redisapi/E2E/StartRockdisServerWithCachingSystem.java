...
public class StartRockdisServerWithCachingSystem {
    ...
    public void invokeRespServer(String ip, int port,
                                 String primaryDir, String secondDir,
                                 boolean enableCachingSystem, int cacheEvictTimePeriod) {
        RespServer server = RespServer.builder()
                .host(ip).port(port)
                .persistenceDir(primaryDir, secondDir)
                .isAuthEnabled(true).getAuthString("admin")
                .enableCacheSystem(enableCachingSystem)
                .setCacheEvictTimePeriod(cacheEvictTimePeriod)
