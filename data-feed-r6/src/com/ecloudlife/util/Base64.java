...
/*
        try
        {
            // Test encoding/decoding byte arrays
            {
                ...
                Log.writeDebug(0, "Bytes 2,2,3,0,9 as Base64: " + encodeBytes( bytes1 ) );
                Log.writeDebug(0, "Bytes 2,2,3,0,9 w/ offset: " + encodeBytes( bytes2, 1, bytes2.length-1 ) );
                byte[] dbytes = decode( encodeBytes( bytes1 ) );

                String sMsg = encodeBytes( bytes1 ) + " decoded: ";
                for( int i = 0; i < dbytes.length; i++ )
                    sMsg.concat( dbytes[i] + (i<dbytes.length-1?",":"\n") );
                Log.writeDebug(0, sMsg);
