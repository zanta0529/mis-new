...
final String referer,final HttpSession session, final JspWriter out) throws Exception {
    ...
    if (StockInfoManager.execJspSet.contains(ex_key))
        return;
    else
        StockInfoManager.execJspSet.add(ex_key);
        ...
        public void run() {
            try {
                ...
                if (referer.endsWith("oddTrade.jsp")) {
                    long delays = StockInfoManager.getUserDelayMillis("od");
                    if (delays == 0)
                        delays = 5000;
                ...
                StockInfoManager.execJspSet.remove(ex_key);
    ...
    if (sync0) {
        ...
        StockInfoManager.jspUpdate.submit(run);
    }
}
%>
